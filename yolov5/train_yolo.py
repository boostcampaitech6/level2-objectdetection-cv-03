...
def parse_opt(known=False):
    ...
    # Logger arguments
    parser.add_argument('--entity', default='ai-tech-6th-funfun', help='Entity')
...
def yolo_main(config=None):
    opt = parse_opt(known=True)
    if config is not None:
        yolo_root_path = '/data/ephemeral/home/level2-objectdetection-cv-03/yolov5'
        opt.epochs = config['epochs']
        opt.batch_size = config['batch_size']
        opt.img_size = config['image_size']
        opt.optimizer = config['optimizer']
        opt.name = config['experiment_name']
        opt.weights = os.path.join(yolo_root_path, config['yolo_model_name']+'.pt')
        opt.data = os.path.join(f'{yolo_root_path}/data', config['yolo_yaml_name'])
    main(opt)
